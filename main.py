...
class FFNeuralNetwork:
    ...
    def backward(Y, Y_pred):
        ...
        for i in reversed(range(len(self.weights))):
            ...
            self.grad_weights[i] = error * derivative(self.output[i])

    # delta_weights array of matriks diinit 0 pas awal banget
    def update_weight(l_rate, delta_weights, momentum):
    ...
    def fit(X, Y, epoch, l_rate, momentum, batch_size):
        for i in range(epoch):
            minibatches = create_minibatches(X, Y, batch_size)
            delta_weights = init_delta_weights()
            for minibatch in minibatches:
                X_mini, Y_mini = minibatch
                for row in X_mini:
                    Y_pred = feed_forward(row.transpose())
                    backward(Y_mini, Y_pred)
                update_weight(l_rate, delta_weights, momentum)
